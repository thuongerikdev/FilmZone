...
namespace FZ.Movie.ApplicationService.Search
{
    ...
    public sealed class MovieIndexService : IMovieIndexService
    {
            ...
            //public async Task ReindexAllMoviesAsync(CancellationToken ct = default)
            //{
            //    // Lấy tất cả movieID từ database
            //    var ids =  _db.Movies.AsNoTracking().Select(x => x.movieID).ToListAsync(ct);
            //    // Xử lý theo lô (batch) để tránh tải quá nhiều vào bộ nhớ
            //    foreach (var batch in (await ids).Chunk(100))
            //    {
            //        await BulkIndexByIdsAsync(batch, ct);
            //    }


            //}
            public async Task<(int total, int batches)> ReindexAllMoviesAsync(CancellationToken ct = default)
            {
                ...
                await foreach (var id in _db.Movies
                    .AsNoTracking()
                    .Select(x => x.movieID)
                    .AsAsyncEnumerable()
                    .WithCancellation(ct))
                {
                    ...
                    {
                        await BulkIndexByIdsAsync(buffer, ct);
                        total += buffer.Count;
                        batches++;
                        buffer.Clear();
                    }
