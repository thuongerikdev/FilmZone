...
import { deleteEpisode, deleteEpisodeSource, getEpisodeSourcesByEpisodeId } from "../services/api";
...
const EpisodeDetailModal = ({ open, onClose, episode, movieTitle, onDeleteSuccess, onUpdateSuccess }) => {
  ...
  const fetchEpisodeSources = async () => {
    setLoading(true);
    try {
      const response = await getEpisodeSourcesByEpisodeId(episode.episodeID);
      if (response.data.errorCode === 200) {
      ...
      renderCell: ({ row }) => (
        <Box display="flex" justifyContent="center" alignItems="center" height="100%" gap={0.5}>
          ...
          <Tooltip title="Chi tiết & Subtitle">
            <IconButton onClick={() => handleOpenSourceDetail(row)} sx={{ color: colors.blueAccent[300] }}>
